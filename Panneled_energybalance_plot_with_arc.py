...
directory4_arc_DEFAULT = '/Users/tijmen/Documents/Tijmen/Climate_Physics/Thesis_local/Data/RACMO_2.4/PXARC11/NC_DEFAULT/'
directory4_arc_MD = '/Users/tijmen/Documents/Tijmen/Climate_Physics/Thesis_local/Data/RACMO_2.4/PXARC11/NC_MD/'
remapdir = '/Users/tijmen/Documents/Tijmen/Climate_Physics/Thesis_local/Data/Remap/'

RC4_ARC_SENSIBLE = (data.Variable_Import(directory4_arc_MD,'hfss'))
RC4_ARC_LATENT = (data.Variable_Import(directory4_arc_MD,'hfls'))
RC4_ARC_LW_D = (data.Variable_Import(directory4_arc_MD,'rlds'))
RC4_ARC_LW_U = -(data.Variable_Import(directory4_arc_MD,'rlus'))
RC4_ARC_SW_D = (data.Variable_Import(directory4_arc_MD,'rsds'))
RC4_ARC_SW_U = -(data.Variable_Import(directory4_arc_MD,'rsus'))
RC4_ARC_GRND = (data.Variable_Import(directory4_arc_MD,'botflx'))
RC4_ARC_RFR = (data.Variable_Import(directory4_arc_MD,'rfrzgl'))
RC4_ARC_SNM = data.Variable_Import(directory4_arc_MD,'snm')
RC4_ARC_SNWD = data.Variable_Import(directory4_arc_DEFAULT,'snd')
RC4_ARC_ALB = data.Variable_Import(directory4_arc_DEFAULT,'sswalb')
RC4_ARC_TILE = data.Variable_Import(directory4_arc_DEFAULT,'tilefrac8')
RC4_ARC_TAS = data.Variable_Import(directory4_arc_DEFAULT,'tas')
...
"""RACMO 4 ARCTIC DOMAIN PLOTTING"""

axs[0,0].plot(RC4_ARC_LW_D.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='orange',label='Down')
axs[0,0].plot(RC4_ARC_LW_U.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='blue',label='Up')
axs[0,0].plot((RC4_ARC_LW_U+RC4_ARC_LW_D).isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='black',label='net')
axs[0,0].set_title('Longwave')



axs[1,0].plot(RC4_ARC_SW_D.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='orange',label='Down',linestyle=(0,(6,6)))
axs[1,0].plot(RC4_ARC_SW_U.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='blue',label='Up',linestyle=(0,(6,6)))
axs[1,0].plot((RC4_ARC_SW_U+RC4_ARC_SW_D).isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='black',label='net',linestyle=(0,(6,6)))
axs[1,0].set_title('Shortwave')


# axs[0,1].plot(RC4_ARC_SNWD.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='Blue',label='Snowheight',linestyle=(0,(6,6)))
# axs2=axs[0,1].twinx()
# axs2.plot(RC4_ARC_SNM.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='Orange',label='Snowmelt',linestyle=(0,(6,6)))
# axs[0,1].set_title('Snowheight')


axs[1,1].plot(RC4_ARC_ALB.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='maroon',label='Albedo',linestyle=(0,(6,6)))
axs[1,1].set_title('Albedo')

axs[1,2].plot(RC4_ARC_TAS.isel(rlon=rlon,rlat=rlat).sel(time=slice(t_start,t_stop)).values,color='grey',label='temperature',linestyle=(0,(6,6)))
axs[1,2].set_title('Temperature')
...
axs[0,2].plot(RACMO_snow_cover['tile_moving_average'].isel(cols=col,rows=row).sel(time=slice(t_start,t_stop)),color='Orange',label='Racmo')
axs[0,2].legend()
